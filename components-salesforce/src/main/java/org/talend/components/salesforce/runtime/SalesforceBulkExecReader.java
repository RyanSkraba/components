...
final class SalesforceBulkExecReader extends SalesforceReader {
	...
	protected Schema getSchema() throws IOException {
		if (querySchema == null) {
			TSalesforceBulkExecProperties sprops = (TSalesforceBulkExecProperties) properties;
			// TODO check the assert : the output schema have values even when
			// no output connector
			querySchema = (Schema) sprops.schemaFlow.schema.getValue();

			// tsalesforcebulkexec don't support dynamic in fact, only
			// tsalesforceoutputbulk support.

			/*
			 * BulkResult currentRow = currentBatchResult.get(resultIndex);
			 * Set<String> keys = currentRow.values.keySet();
			 * 
			 * List<Schema.Field> columnlist = new ArrayList<>(); for
			 * (Schema.Field se : querySchema.getFields()) { if
			 * (keys.contains(se.name())) { columnlist.add(new
			 * Schema.Field(se.name(), se.schema(), se.doc(), se.defaultVal()));
			 * } }
			 * 
			 * Map<String, Object> objectProps = querySchema.getObjectProps();
			 * querySchema = Schema.createRecord(querySchema.getName(),
			 * querySchema.getDoc(), querySchema.getNamespace(),
			 * querySchema.isError());
